...
class LocalSensitiveAttention(nn.Module):
    def __init__(self, 
        ...
        self.W = nn.Linear(decoder_hidden_size, attention_dim, bias=True)
    ...
    def _cal_energy(self, query, values, cumulative_attention_weights, mask=None):
        ...
        location_features = self.F(cumulative_attention_weights.unsqueeze(1))
        
        Uf = self.U(location_features.transpose(1,2))

        energies = self.v(torch.tanh(Ws + self.Vh + Uf)).squeeze(-1)
    ...
    def forward(self, query, values, cumulative_attention_weights, mask=None):
        ...
        energies = self._cal_energy(query, values, cumulative_attention_weights, mask)
...
class Decoder(nn.Module):
    def __init__(self, config):
        ...
        self.postnet = PostNet(self.config.num_mels)
...
if __name__ == "__main__":
    ...
    for text_padded, input_lengths, mel_padded, gate_padded, encoder_mask, decoder_mask in loader:
        ...
        encoded_outputs = encoder(text_padded)
        decoder(encoded_outputs, encoder_mask, mel_padded, decoder_mask)
